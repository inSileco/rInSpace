...
dir.create("custommaps/data")

can1 <- getData("GADM", country = "CAN", level = 1, path = "custommaps/data")
usa0 <- getData("GADM", country = "USA", level = 0, path = "custommaps/data")
```

Transform to `sf` objects to facilitate manipulation

```{r st_sf, message = FALSE}
...
temp <- getData("worldclim", var = "tmean", res = 10, path = "custommaps/data")
...
```{r, eval = TRUE, fig.width = 5.5, fig.height = 6}
...
dir.create("custommaps/data", showWarnings = FALSE)
envCov <- sdmpredictors::load_layers(layers, datadir = "custommaps/data", rasterstack = TRUE)
...
values(envCov[[2]]) <- values(envCov[[2]]) / max(values(envCov[[2]]), na.rm = TRUE)
```
...
<center>![](custommaps/example1.png){width=80%}</center>
...
<center>![](custommaps/layout1.png){width=90%}</center>
...
<center>![](custommaps/example2.png){width=90%}</center>

And here is the actual figure!

<center>![](custommaps/layout2.png){width=90%}</center>
