...
# Read csv file in R
pts <- read.csv("data/ruisso.csv", header = TRUE, dec = ",")
```
...
st_write(pts_sf, "data/pts_sf.shp", delete_dsn = TRUE)
```
...
We now import raster data as a [.tif file](https://en.wikipedia.org/wiki/TIFF)
file. Here we chose the data for Rouville in 2015 from the collection of tif files representing the [canopy index](https://observatoire.cmm.qc.ca/produits/donnees-georeferencees/#indice_canopee) computed by the Montreal Metropolitain Community (CMM) from an NDVI index and an elevation surface model, represents the low vegetation cover, the high vegetation cover of more than 3 meters (the canopy), the low mineral surfaces and the high mineral surfaces more than 3 meters (roof).
